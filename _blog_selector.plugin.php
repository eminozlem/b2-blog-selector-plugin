...
class blog_selector_plugin extends Plugin
{
	...
	*/
	...
	{	
		...
		require_css( $plugins_url.'blog_selector_plugin/rsc/chosen.override.css', true );
		require_js( $plugins_url.'blog_selector_plugin/rsc/chosen.jquery.min.js', true );
		add_js_headline("
			$.noConflict();
			jQuery(document).ready(function($){
			  $('select.blog_sel_w').chosen();
	...
	function disp_coll_select( $params )	{
		...
		{	// Loop through all public blogs:
			...
			$select_options .= '<option data-url="'.$l_Blog->gen_blogurl().'" value="'.$l_blog_ID.'" title="'.$l_Blog->dget( 'description', 'formvalue' ).'"';
